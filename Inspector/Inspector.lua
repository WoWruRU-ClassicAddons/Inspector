...
function InspectorFrame_OnLoad()
	...
	chatMessage( version..INSPECTOR_LOAD, 0.2, 0.6, 1.0 );
	chatMessage( INSPECTOR_LOAD2,  0.2, 0.6, 1.0 );
...
function InspectorHideHonor()
	...
	InspectorSwitchMode:SetText(INSPECTOR_BUTTON_HONOR);
...
function InspectorShowHonor()
	InspectorMode = "Honor";
	InspectorSwitchMode:SetText(INSPECTOR_BUTTON_MODEL);
...
function InspectorHelp()
	...
	chatMessage(INSPECTOR_CMD1, 1.0, 1.0, 1.0);
	chatMessage(INSPECTOR_CMD2, 1.0, 1.0, 1.0);
	chatMessage(INSPECTOR_CMD3, 1.0, 1.0, 1.0);
end
...
function InspectorInspect()
	...
	InspectorPlayerInfoText:SetText( targetRace..", "..targetClass.." "..targetLevel..INSPECTOR_PLAYER_LEVEL );
	-- Get guild info about the target
	targetGuild, targetGuildRank = GetGuildInfo("target");
	if( targetGuild and targetGuildRank ) then
		InspectorPlayerGuildText:SetText( targetGuildRank..INSPECTOR_PLAYER_GUILD..targetGuild );
	else 
	...
	if( targetRank and targetRankName ) then
		-- Set the text
		InspectorPlayerPVPRankText:SetText( targetRankName..INSPECTOR_PLAYER_RANK..targetRank..")" );
		-- Set the texture
		InspectorPlayerPVPRankTexture:SetTexture( format("%s%02d", "Interface\\PvPRankBadges\\PvPRank", targetRank) );
	else
		InspectorPlayerPVPRankText:SetText( INSPECTOR_PLAYER_0RANK );
	...
	if( CheckInteractDistance("target",1) ) then
		...
		InHonorRange = 1;
		InspectorPlayerPVPRangeText:SetText( INSPECTOR_REQ_HONOR );
		InspectorPlayerPVPRangeText:SetTextColor(1.0, 0.82, 0.0);
	else
		InHonorRange = 0;
		InspectorPlayerPVPRangeText:SetText( INSPECTOR_MOVE_CLOSER );
			...
			if( InHonorRange == 1 ) then
				...
				if( HasInspectHonorData() ) then				
					...
					if( rankName and rankNumber ) then
						...
						InspectorPlayerPVPLifetimeRank:SetText( INSPECTOR_LIFETIME_0RANK );
					end
					-- We don't need to get the honor data again,
					--  so set the waiting variable to 0 (false)
					WaitingForHonor = 0;
					honorRequested = 0;
					InspectorPlayerPVPRangeText:SetText( INSPECTOR_STATS_LOADED );
				...
				elseif( timePassedTotal > honorRequestTimeout ) then
					...
					RequestInspectHonorData();
					honorRequested = 1;
					InspectorPlayerPVPRangeText:SetText( INSPECTOR_RE_REQ_HONOR );
					InspectorPlayerPVPRangeText:SetTextColor(1.0, 0.82, 0.0);
					timePassedTotal = 0;
				end
			end
